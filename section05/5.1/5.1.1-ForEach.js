...
data.forEach(user =>
  phrases.push(`${user.company.catchPhrase} ${user.company.bs}`)
);

console.log(phrases);
// query the HTML for the class class-week, then use forEach to change the text content
let classWeek = document.querySelectorAll(".Class-week");
classWeek.forEach(
  htmlElement => (htmlElement.textContent = `"I changed my text content."`)
);
